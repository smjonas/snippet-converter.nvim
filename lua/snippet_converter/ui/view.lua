...
M.register_keymap = function(lhs, handler_fn)
  global_keymaps[lhs] = handler_fn
end
...
M.new_window = function()
  ...
  local function open()
    ...
    local resize_autocmd = (
      "autocmd VimResized <buffer> lua require('snippet_converter.ui.view').redraw_window(%d)"
    ):format(win_id)

    -- Will close the window when clicked outside
    local autoclose_autocmd = (
      "autocmd WinLeave,BufHidden,BufLeave <buffer> ++once lua require('snippet_converter.ui.view').destroy_window(%d, %d)"
    ):format(win_id, bufnr)
  ...
  local draw = function(model)
    ...
    -- Set line contents
    local lines, highlights = model.lines, model.highlights
    ...
    for lhs, _ in pairs(global_keymaps) do
      vim.api.nvim_buf_set_keymap(
        ...
        lhs,
        string.format(
          "<cmd>lua require('snippet_converter.ui.view').handle_keymap(%q)<cr>",
