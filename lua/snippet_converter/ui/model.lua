...
function Model:submit_task(source_format, num_snippets, num_input_files)
...
function Model:complete_task(
  ...
  self.tasks[source_label].failures[target_label] = failures
  self.tasks[source_label].num_output_files[target_label] = num_output_files
  self.max_num_failures = math.max(self.max_num_failures or 0, #failures)
end
